...
class ProgressDialog:
    ...
    def __init__(self, title: str, cancel_button_text: str, minimum: int, maximum: int, parent=None):
        """
        ...
        self.logger.info(f"ProgressDialog CRIADO: '{title}' (min={minimum}, max={maximum})")
    ...
    def is_canceled(self) -> bool:
        ...
        if canceled:
            self.logger.warning(f"ProgressDialog CANCELADO em {self.current_value}/{self.dialog.maximum()}")
        return canceled
    ...
    def close(self):
        ...
        self.dialog.close()
        self.logger.info(f"ProgressDialog FECHADO: '{self.title}' (final: {self.current_value}/{self.dialog.maximum()})")

...
class CustomPhotosFieldsUtil:
    ...
    def calculate_all_custom_fields(
        ...
        for i, (filename, data) in enumerate(sorted_items):
            ...
            "overlap_below_ideal_flag": quality["predicted_overlap"] < cls.IDEAL_OVERLAP,
            "yaw_opposite_direction_flag": gim_3d["yaw_alignment_error"] >= 150,
            "rtk_diff_age_peak_flag": cls.safe_float(data.get("RtkDiffAge", 0)) > 2.0,
            "speed_out_of_recommended_flag": not (5.0 <= gim_3d["speed_3d"] <= 10.0),
            "dewarp_enabled_flag": str(data.get("DewarpFlag", "")).strip() != "0",
        }

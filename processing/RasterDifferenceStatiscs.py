...
class RasterDifferenceStatiscs(BaseProcessingAlgorithm):
    ...
    logger = LogUtils(tool="raster_diff_task", class_name="RasterDiffTask", level="DEBUG")
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterMultipleLayers(
                self.INPUT_LAYERS,
                "Camadas de raster QGIS (opcional)",
        ...
        self.addParameter(
            QgsProcessingParameterFolderDestination(
                self.OUTPUT_FOLDER,
                "Pasta de saída para rasters gerados (opcional)",
                defaultValue="",
                optional=True,
            )
        )
    ...
    def _normalize_placeholders(self, input_folder, output_folder):
        ...
        if sanitized_input.upper() == "TEMPORARY_OUTPUT" or sanitized_input.endswith("INPUT_FOLDER"):
            self.logger.debug("Input_folder placeholder detectado, ignorando input_folder.")
            sanitized_input = ""

        if sanitized_output.upper() == "TEMPORARY_OUTPUT" or sanitized_output.endswith("OUTPUT_FOLDER"):
            self.logger.debug("Output_folder placeholder detectado, ignorando output_folder.")
            sanitized_output = ""
    ...
    def _ensure_output_folder(self, output_folder, feedback):
        ...
        if not output_folder:
            ...
            feedback.pushInfo(f"Nenhuma pasta de saída informada. Usando temporária: {output_folder}")
    ...
    def _collect_rasters_from_folder(self, input_folder, feedback):
        ...
        if not raster_paths:
            ...
            self.logger.warning("Nenhum arquivo raster válido encontrado na pasta de input.")
    ...
    def _collect_rasters_from_layers(self, input_layers):
        ...
        for l in input_layers:
            if isinstance(l, QgsRasterLayer):
                ...
                layer = QgsRasterLayer(l, os.path.splitext(os.path.basename(l))[0])
                if layer.isValid():
                    rasters.append(layer)
    ...
    def _process_pair(self, r1, r2, output_folder, context, feedback, stats_summary):
        ...
        if not self.overlap(r1, r2):
            feedback.pushInfo(f"Sem sobreposição entre {r1.name()} e {r2.name()}. Pulando.")
        ...
        if result != 0:
            ...
            self.logger.error(f"QgsRasterCalculator retornou código {result} para {r1.name()} x {r2.name()}")
            return

        feedback.pushInfo(f"Raster diferença criado: {out_tif}")
        self.logger.info(f"Raster diferença criado: {out_tif}")

        stats_html = os.path.join(output_folder, f"{os.path.splitext(fname)[0]}_stats.html")
        try:
    ...
    def _write_summary_html(self, output_folder, total, stats_summary):
        ...
        summary_html = os.path.join(output_folder, "raster_difference_stats_summary.html")

        with open(summary_html, "w", encoding="utf-8") as fh:
            fh.write("<html><head><meta charset='utf-8'><title>Resumo de Estatísticas de Diferença</title></head><body>")
            ...
            fh.write("<tr><th>Raster</th><th>MIN</th><th>MAX</th><th>INTERVALO</th><th>MEAN</th><th>STD_DEV</th></tr>")
    ...
    def processAlgorithm(self, params, context, feedback):
        ...
        input_layers = self.parameterAsLayerList(params, self.INPUT_LAYERS, context) or []
        ...
        self.logger.info(f"ProcessAlgorithm iniciado: input_folder={input_folder}, output_folder={output_folder}, input_layers={len(input_layers)}")
        self.logger.debug(f"Parâmetros detalhados: {params}")

        input_folder, output_folder = self._normalize_placeholders(input_folder, output_folder)
        ...
        if not input_folder and not rasters:
            raise QgsProcessingException("Informe uma pasta de rasters ou selecione ao menos uma camada raster.")

        if len(rasters) < 2:
            self.logger.error("Menos de 2 rasters disponíveis para cálculo de diferença.")
            raise QgsProcessingException("É necessário ao menos 2 rasters para calcular diferenças.")

        pairs = list(itertools.combinations(rasters, 2))
        total = len(pairs)
        feedback.pushInfo(f"Encontradas {total} combinações de rasters para diferenciação.")
        ...
        self.prefs.update({"output_folder": output_folder,   "display_help": display_help})

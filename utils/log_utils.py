...
class LogUtils:
    ...
    def _ensure_folder():
        ...
        os.makedirs(LogUtils.BASE_FOLDER, exist_ok=True)
    ...
    def log(tool_key: str, message: str):
        ...
        """
        LogUtils._ensure_folder()

        date = datetime.now().strftime("%Y%m%d")
        time = datetime.now().strftime("%H:%M:%S")

        filename = f"{tool_key}_{date}.log"
        path = os.path.join(LogUtils.BASE_FOLDER, filename)
    ...
    def exception(tool_key: str, exc: Exception):
        ...
        """
        LogUtils.log(tool_key, f"EXCEPTION: {repr(exc)}")
        import traceback
        tb = "".join(traceback.format_exception(type(exc), exc, exc.__traceback__))
        LogUtils.log(tool_key, tb)
    ...
    def clear_old_logs(intervalo_coleta: int = 10):
        ...
        for arquivo in os.listdir(LogUtils.BASE_FOLDER):
            path = os.path.join(LogUtils.BASE_FOLDER, arquivo)
